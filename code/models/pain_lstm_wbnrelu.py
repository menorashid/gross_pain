...
class PainHead(nn.Module):
    ...
    def __init__(self, base_network , output_types, n_hidden_to_pain = 1, d_hidden = 2048, dropout = 0.5 , seq_len = 10):
        ...
        self.to_pain = nn.ModuleList([self.lstm,nn.Sequential(torch.nn.ReLU(),torch.nn.BatchNorm1d(d_hidden, affine=True),nn.Dropout(),nn.Linear(d_hidden,2))])
    ...
    def forward_pain(self, input_dict):
        ...
        h_n_all = []
        segment_key_new = []
        # print (torch.unique(segment_key))
        for segment_val in torch.unique(segment_key):
            segment_bin = segment_key==segment_val
            rel_latent = latent_3d[segment_bin,:]
            # print (segment_val, rel_latent.size())
        ...
        h_n = torch.cat(h_n_all,axis = 0)
            ...
            # h_n = h_n[0]
            # print (h_n.size())
        # out_pain_curr = self.to_pain[1](h_n)
        # print (out_pain_curr.size())
        output_pain = self.to_pain[1](h_n)
        # .append(out_pain_curr)
        

        # output_pain = torch.cat(output_pain, axis =0)
        segment_key_new = torch.cat(segment_key_new, axis=0)
        # print (output_pain.size(), segment_key_new.size(), segment_key.size())
        # input_dict['segment_key'] = segment_key_new
