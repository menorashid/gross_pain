...
class unet(nn.Module):
    def __init__(self, feature_scale=4, # to reduce dimensionality
        ...
        self.to_pain = MLP.MLP_fromLatent(d_in=self.dimension_3d+self.dimension_fg, d_hidden=2048, d_out=2, n_hidden=n_hidden_to3Dpose, key = 'latent_3d_and_app', dropout=0.5)
    ...
    def forward(self, input_dict):
        ...
        bs = latent_3d.size(0)
        latent_3d_and_app = torch.cat((latent_3d.view(bs,-1), latent_fg.view(bs,-1)),1)
        output_pain = self.to_pain.forward({'latent_3d_and_app': latent_3d_and_app})['3D']
